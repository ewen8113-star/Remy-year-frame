...
- **交接日**：2026-05-19（周二）
- **下一工作日**：2026-05-20（周三，优先修复活动报价保存后列表金额为空；排期日历增加客户与场地展示；其余报价/日历功能后续排期）。

---

## 当天完成

- **2026-05-19 版本留痕（工作日志）— 活动报价 · 多场报价**
  - **多场报价模块（前后端骨架）**
    - 数据库：`quote_mode`（single / multi）、`linked_sessions`（JSON）；迁移 `npm run migrate:quotation-multi`（`src/scripts/addQuotationMultiSession.js`）。
    - 后端：`src/routes/quotation.js` 支持多场 POST/PUT；`src/quotation/multiSummaryItems.js` 每场 5 项费用 + 行内小计/服务费 10%/税费 6%/合计；`src/quotation/ensureQuotationTables.js`。
    - 前端：`public/activity-quotes.js` 独立模块；列表「新建单场报价」「新建多场报价」；多场 Summary 表格 **一行一场**（项目编号 + 自动字段 + 5 项手填费用 + 行内合计 + 底部汇总）。
    - 列表「项目编号」多行展示：≤3 场全显，≥4 场显示首场 / `︙` / 末场。
  - **多场编辑体验（本轮 UI 修复）**
    - 项目编号：由自定义 combobox 改为与 **报销登记相同的 `<datalist>`**（`#aqMultiProjectList`），避免表格 `overflow` 裁切下拉；列宽加至约 260–340px。
    - 日期 / 城市 / 客户名称 / 类型：改为 **纯文本**（`.aq-auto-val`），选中项目编号后自动带出，样式与小计列一致。
    - 空草稿：新建多场后未编辑点「返回列表」或「取消」→ 自动 `DELETE` 删除空记录；编辑页增加 **「取消」** 按钮；`multiDraftPristine` 状态跟踪。
  - **静态资源版本**：`activity-quotes.js?v=20260519-aq-v9`。
  - **影响文件（未提交，工作区有改动）**：`public/activity-quotes.js`、`public/index.html`、`public/style.css`、`src/routes/quotation.js`、`src/routes/dashboard.js`、`src/server.js`、`package.json` 及 quotation 相关新建文件。

---

## 明天优先（Bug / 需求）

1. **【Bug】新建报价后金额未保存**：保存后报价列表中金额/合计等显示为空，需排查 PUT 载荷、`quotation_items` 写入、列表 API 返回字段与前端 `fmtMoney` 绑定。
2. **【需求】排期日历**：在日历视图增加 **客户**、**场地** 字段的内容展示（具体展示位置/样式待产品确认）。
3. **后续开发**：单场报价完善、PDF 导出验收、其他报价模块需求 — **明天再排**。
...
- **活动报价（2026-05-19 收工遗留）**
  - 新建/保存报价后列表金额为空（见上「明天优先」#1）。
  - 多场报价：非 admin 用户取消空草稿时 `DELETE` 可能失败（当前仅 admin 可删），若需 viewer 也能放弃草稿需再定规则。
  - 排期日历客户/场地展示未开始。
- **入库单台账（2026-04-21）**
...
1. **活动报价**：修复保存后列表金额为空；验收单场/多场保存、列表、预览、PDF。
2. **排期日历**：增加客户、场地字段展示。
3. 场次报价明细 + 区域版 Excel 报价模板导出（延续原 backlog）。
4. 「修改密码（本人）」与「管理员重置密码」。
...
> 活动报价多场模式已可编辑（datalist 选项目编号、一行一场费用表）；**明天先修列表金额不显示**；排期日历待加客户/场地；其余报价功能后续排期。
